...
class DataBase:
    """
    - `create` inserts records from 'path2csv' to the sqlite
      database 'path'
    - `query` can be used to query the database and
      return the result as pd.DataFrame.
    - `close` closes the connection.
    """

    def __init__(self, path2csv: str = "data/_csv") -> None:
    ...
    def _read_tables_from_date_folder(self, folder: str) -> dict[str, pd.DataFrame]:
        """
        The name of the folder must be a valid date, e.g., folder =
        'data/csv/2019_12_31.'
        """
        ...
        for file in files:
            ...
            if format == "zip":
                ...
                print(
                    ...
                    f" {os.path.join(self.path, name)} until {folder_name}"
                )
            else:
    ...
    def _read_csv(self, path: str, format_: str = "zip"):
        return pd.read_csv(path, sep=";", compression=format_)  # type: ignore
